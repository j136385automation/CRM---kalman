...
export function MessageComposer({
  ...
  return (
    <form onSubmit={onSubmit} className='space-y-2 sm:space-y-3' aria-label='Responder conversa'>
      <label htmlFor='robo-editor' className='sr-only'>
      ...
      <div className='border-border/40 bg-background/80 flex items-end gap-2 rounded-2xl border p-3 backdrop-blur sm:gap-3 sm:rounded-3xl sm:p-4'>
        <div className='min-w-0 flex-1'>
          ...
          <Textarea
            ...
            placeholder={
              'Responder ' + contactName + ' (Enter para enviar, Shift+Enter para nova linha)'
            }
            rows={2}
            ...
            aria-label={'Responder ' + contactName}
          />
        ...
        <div className='flex shrink-0 flex-col items-end gap-1.5 sm:w-24 sm:gap-2'>
          <input
            ...
            onChange={(e) => {
              ...
              // Reseta o input para permitir anexar o mesmo arquivo novamente
              e.target.value = '';
            }}
          />
          <Button
