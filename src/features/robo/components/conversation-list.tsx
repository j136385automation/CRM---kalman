  ...
  return (
    <div className='border-border/40 bg-background/75 hidden h-full flex-col gap-4 overflow-hidden rounded-2xl border p-3 backdrop-blur lg:col-start-1 lg:col-end-2 lg:flex lg:rounded-3xl lg:p-4'>
      <div className='flex items-center justify-between gap-3'>
        <div>
          <p className='text-foreground text-sm font-semibold'>Robô WhatsApp</p>
          <p className='text-muted-foreground text-xs'>
            {conversations.length} conversa{conversations.length === 1 ? '' : 's'} ativa
      ...
      <label htmlFor='robo-search' className='sr-only'>
        Buscar conversas
      </label>
          ...
          return (
            ...
            >
              ...
              <div className='min-w-0 flex-1 space-y-1'>
                ...
                <Badge
                  variant={conversation.respondidaPor === 'robo' ? 'default' : 'secondary'}
                  className='pointer-events-none h-5 rounded-sm px-1.5 text-[11px]'
                >
                  {conversation.respondidaPor === 'robo'
                    ? 'Respondida pelo robô'
                    : 'Atendimento humano'}
                </Badge>
              </div>
